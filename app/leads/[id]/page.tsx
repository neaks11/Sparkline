...
export default function LeadDetailPage() {
  ...
  const medSpaLead = isMedSpaNiche(lead.niche);
      ...
      <section className="card p-6">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            {medSpaLead && <p className="mt-1 text-xs text-slate-500">Med spa outreach context: prioritize consultation booking flow and front desk follow-up efficiency.</p>}
          </div>
          <div className="flex gap-2">
        ...
        {lead.medSpaSignals.length > 0 && (
          <div className="mt-5">
            <h2 className="font-semibold">Conversion Signals</h2>
            <ul className="mt-2 list-disc space-y-1 pl-5 text-sm text-slate-700 dark:text-slate-300">
              {lead.medSpaSignals.map((signal) => <li key={signal}>{signal}</li>)}
            </ul>
          </div>
        )}

        <div className="mt-5">
          <h2 className="font-semibold">Notes / Offer Context</h2>
